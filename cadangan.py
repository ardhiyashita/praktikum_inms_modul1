...
def read_transaksi(): #digunakan untuk membaca isi dalam tabel
        ...
        for i in mycursor.fetchall(): #mengambil masing-masing data dengan menggunakan fetch sehingga data tampil per row/baris
            ...
            updated_at = i[7]
            delete_at = i[8]
            print(str(idx)+" "+pegawai+" "+customer+" "+barang+" "+str(harga)+" "+str(status_transaksi)+" "+str(tgl_transaksi)+" "+str(updated_at)+" "+str(delete_at))
...
def insert_transaksi(): #digunakan untuk menginput data transaksi
    ...
    val = (pegawai, customer, barang, harga, timestamp, None, "pending") #variabel akan melempar value agar masuk ke dalam sintaks diatasnya, karena banyak jadi saya pisahkan
    ...
    # print("Berhasil menambah transaksi pada db host")
    ...
    for i in mycursor.fetchall():
        ...
        if str(sinkronisasi) == "None":
            ...
            if aksi == "insert":
                ...
                sql = "delete from tb_transaksi where id="+id_tabel+" "
                mycursor_host.execute(sql)
                mydb_host.commit()
                print("Berhasil menghapus transaksi pada db host")
                ...
                print("Seluruh data telah tersinkronisasi")
